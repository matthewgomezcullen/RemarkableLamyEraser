...
static inline double getTimeDelta(struct timeval *current, struct timeval *prev) {
...
int get_triggger(struct input_event *ev_pen) {
    ...
    if (!(ev_pen->code == EV_SYN || ev_pen->code == BTN_STYLUS)) { // BTN_TOOL_PEN = 0 is followed by EV_SYN and (possibly) BTN_STYLUS when pulling away pen
      ...
      if (elapsedTime > MAX_CYCLE_TIME) {
        ...
        if (pressHoldSent) {
          ...
          trigger = HOLD_OFF | clicks; // press hold off type message
          pressHoldSent    = false;
        }
        clicks   = 0;
      }
  ...
  if (longClick) {
    ...
    trigger = LCLICK | clicks; // long click type message
    clicks = 0;
    longClick = false;
    possiblyLongClick = false;
  }
    ...
    if (elapsedTime < MAX_CLICK_TIME) {
      ...
      if (!clickRegistered) { // button still held or just released
        if (!pressHoldSent) {
          ...
          trigger = HOLD_ON | clicks; // press hold on type message
          pressHoldSent    = true;
      ...
      if (clickRegistered) {
        ...
        trigger       = CLICK | clicks; // click type message
        clickRegistered = false;
        clicks        = 0;
      }
      if (released) {
